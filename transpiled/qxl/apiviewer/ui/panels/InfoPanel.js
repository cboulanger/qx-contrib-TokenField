(function () {
  ...
  qx.Bootstrap.executePendingDefers($$dbClassInfo);qx.Class.define("qxl.apiviewer.ui.panels.InfoPanel", {
      ...
      toggleShowItemDetails: function toggleShowItemDetails(itemName, fromClassName) {
        try {
          ...
          for (var arr = this.getPanelItemObjects(fromClassNode), i = 0; i < arr.length && !node; i++) {
            var tmp = arr[i];
            if (tmp.getName() == itemName) node = tmp;
          }
          if (!node) return;
...
//# sourceMappingURL=InfoPanel.js.map?dt=1554385339186